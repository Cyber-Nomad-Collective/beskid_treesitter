...
export default grammar({
	...
	rules: {
		...
		item_with_docs: ($) => seq(optional($.doc_comment), $._item),
		...
		_type_path_segment: ($) =>
			seq(field("name", $.identifier), optional($.generic_arguments)),

		primitive_type: (_) =>
			choice("bool", "i32", "i64", "u8", "f64", "char", "string", "unit"),

		_expression: ($) =>
			choice($.lambda_expression, $.match_expression, $.assignment_expression),
		...
		_lambda_parameters: ($) =>
			choice($.identifier, seq("(", optional($.lambda_parameter_list), ")")),
		...
		logical_or_expression: ($) =>
			prec.left(PREC.logical_or, sep1($.logical_and_expression, "||")),
		logical_and_expression: ($) =>
			prec.left(PREC.logical_and, sep1($.equality_expression, "&&")),
		equality_expression: ($) =>
			prec.left(
				PREC.equality,
				sep1($.comparison_expression, choice("===", "!==", "==", "!=")),
			),
		comparison_expression: ($) =>
			prec.left(
				PREC.comparison,
				sep1($.additive_expression, choice("<=", ">=", "<", ">")),
			),
		additive_expression: ($) =>
			prec.left(
				PREC.additive,
				sep1($.multiplication_expression, choice("+", "-")),
			),
		multiplication_expression: ($) =>
			prec.left(PREC.multiplicative, sep1($.unary_expression, choice("*", "/"))),
		...
		unary_expression: ($) =>
			choice(
				...
				prec(
					PREC.unary,
					seq(repeat(choice("-", "!")), field("operand", $.postfix_expression)),
				),
			),
		...
		struct_literal_expression: ($) =>
			prec(
				PREC.primary,
				seq(field("type", $._type_path), "{", optional($.field_value_list), "}"),
			),
		...
		field_with_docs: ($) => seq(optional($.doc_comment), $._field),

		_field: ($) => choice($.value_field, $.event_field, $.inject_field),
		...
		enum_variant_with_docs: ($) => seq(optional($.doc_comment), $.enum_variant),
		...
		parameter_with_docs: ($) => seq(optional($.doc_comment), $._parameter),
		...
		impl_block: ($) =>
			seq("impl", field("receiver", $.receiver_type), field("body", $.impl_body)),
		...
		receiver_type: ($) =>
			choice($.primitive_type, alias($._type_path, $.type_path)),

		impl_body: ($) => seq("{", repeat($.impl_method_with_docs), "}"),

		impl_method_with_docs: ($) => seq(optional($.doc_comment), $.impl_method),
		...
		macro_parameter: ($) =>
			seq(field("kind", $.macro_fragment_kind), field("name", $.identifier)),

		macro_fragment_kind: (_) =>
			choice(
				"block",
				"expression",
				"statement",
		...
		contract_item_with_docs: ($) =>
			seq(optional($.doc_comment), $._contract_item),

		_contract_item: ($) =>
			choice($.contract_method_signature, $.contract_embedding),
		...
		contract_embedding: ($) => seq(field("name", $.identifier), optional(";")),
		...
		_host_body_item: ($) =>
			choice($.registry_block, $.scope_hook, $.scope_definition),
		...
		_scope_body_item: ($) =>
			choice($.registry_block, $.scope_hook, $.scope_definition, $.registry_entry),
		...
		test_body_item_with_docs: ($) =>
			seq(optional($.doc_comment), $._test_body_item),

		_test_body_item: ($) =>
			choice($.test_meta_section, $.test_skip_section, $._statement),

		test_meta_section: ($) => seq("meta", field("body", $.test_meta_body)),
		...
		test_skip_section: ($) => seq("skip", field("body", $.test_skip_body)),
		...
		_pattern: ($) =>
			choice($.wildcard_pattern, $.enum_pattern, $.identifier, $.literal),
		...
		string_literal: ($) =>
			seq(
				'"',
				repeat(choice($.string_interpolation, $.string_escape, $.string_content)),
				'"',
			),
